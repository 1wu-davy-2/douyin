...
**2026-09-15（会话 6）：第二轮真实发送 —— 好友数异常保护 + 重新登录幂等，commit eb0724f**

SSE 字段修复已验证生效（日志可读："账号 gd_random 好友刷新:共 15 人"、"发送批次结束(gd_random):成功 0 · 弱确认 0 · 失败 1 · 共 1"）。本轮新暴露：

1. **好友数异常（高危）**：一次刷新返回 697 人（正常 17），疑似滚动抓到"陌生人私信/全部"tab；整表替换后用户全选会误发给陌生人。修：Go `RefreshFriends` 在整表替换前校验（新增 `Store.CountFriends`）——新数量 >200 且 > 旧数量×3 时拒绝替换并返回明确错误。
2. **重新登录撞 UNIQUE**：重新扫码时 `unique_id` 不变（`web-self-{uid cookie}`，`uid_tt` 固定）→ 409 "account already exists" → 前端报"登录收尾失败"。风控踢下线后用户**无法自助恢复登录态**。修：login-dialog 捕获 409 → 复用已有账号 → 提示"已刷新登录态"，后续照常刷新 cookie/好友。
3. **平台风控（非代码缺陷）**：15:30 好友刷新成功、15:31 发送即被要求登录（body 为完整登录页）；随后探测 profile 登录态完全正常（58 条 cookie、sessionid 未过期、私信页无登录墙）。判定为抖音对"短时间反复开关浏览器"的风控挑战——工程侧只做快速失败与提示，根因靠降低操作频率（与 §10 风控红线一致）。

诊断要点：`spike/probe_chat.py` 可直接区分"profile 真失效"与"平台风控"。
